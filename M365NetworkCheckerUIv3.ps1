...
.DESCRIPTION
   Parses m365 Urs for markdown tables; uses that list to check connectivity
...
<Window
 ...
 WindowStartupLocation="CenterScreen"
    Title="StartSelection" Height="550" Width="800"
    ResizeMode="NoResize">
    <Window.Resources>
        <ResourceDictionary>
    ...
    <Border BorderBrush="#002E8A" BorderThickness="2">
        <Grid Margin="0,0,0,0">
            <Canvas x:Name="s
            tartCanvas" HorizontalAlignment="Left" Height="518" Margin="0,0,-2,0" VerticalAlignment="Top" Width="792" Background="#004275" Panel.ZIndex="300">
                <TextBlock HorizontalAlignment="Center" Text="M365 Endpoint Cloud Selector" VerticalAlignment="Top" FontSize="36" Width="739" TextAlignment="Center" FontFamily="Segoe UI Light" Canvas.Top="29" Canvas.Left="10" Height="64" Foreground="White"/>
                ...
                <Button x:Name="startBegin" Style="{DynamicResource modernButton}" Width="209" Height="69" Canvas.Left="560" Canvas.Top="427">
                ...
                <TextBlock Text="Select this option if there is no connectivity to doc.microsoft.com and gitbub.com. Please consider that this option may reflect the wrong endpoints since is not getting updated data." HorizontalAlignment="Left" Foreground="LightBlue" TextWrapping="Wrap" VerticalAlignment="Top" Width="288" Grid.ColumnSpan="2" Canvas.Left="10" Canvas.Top="439"/>
            </Canvas>
        </Grid>
    </Border>
</Window>
...
[string]$XAML = $startXAML -replace 'mc:Ignorable="d"','' -replace "x:N",'N' -replace '^<Win.*', '<Window' -replace 'Click=".*','/>'
...
$UI.Topmost = $true
$UI.Title = 'M365 Endpoint Cloud Selector'

@('WorldWide','DoD','GCC','GCC High','China','Germany') | ForEach-object {$ui_startCloudListBox.Items.Add($_)} | Out-Null
#preselect item if argument used
If($Cloud){
    $ui_startCloudListBox.SelectedItem = $Cloud
    $m365Hash = Get-M365Hash -Selection $Cloud
    $IntuneHash = Get-IntuneHash -Selection $Cloud
}

$ui_startOfflineMode.IsChecked = $OfflineMode

#Button Actions
#========================================
$ui_startOfflineMode.Add_Checked({
    $global:OfflineMode = $true
})
$ui_startOfflineMode.Add_UnChecked({
    $global:OfflineMode = $false
})

$ui_startCloudListBox.Add_SelectionChanged({
    $global:Cloud = $ui_startCloudListBox.SelectedItem
    $global:m365Hash = Get-M365Hash -Selection $Cloud
    $global:IntuneHash = Get-IntuneHash -Selection $Cloud
})

$ui_startBegin.Add_Click({
    If($DebugMode){
        write-host ("Cloud is now: {0}" -f $Cloud)
        write-host ("OfflineMode is now: {0}" -f $OfflineMode)
        write-host ("m365Hash is now: {0}" -f $m365Hash.sourceUri)
        write-host ("IntuneHash is now: {0}" -f $IntuneHash.sourceUri)
    }
    $UI.Close() | Out-Null
})

# if Cloud Parameter is used, assume user knows and
# don't display start UI
#============================================
If($PSBoundParameters.ContainsKey('Cloud') )
{
    ...
    $global:m365Hash = Get-M365Hash -Selection $Cloud
    $global:IntuneHash = Get-IntuneHash -Selection $Cloud
    $global:OfflineMode = $OfflineMode
}
Else{
    Show-UIMenu
}
...
#Begin Network Checker UI within multithreaded runspace
#===========================================================================
...
$Runspace.SessionStateProxy.SetVariable("m365Hash",$global:m365Hash)
$Runspace.SessionStateProxy.SetVariable("intuneHash",$global:IntuneHash)
$Runspace.SessionStateProxy.SetVariable("OfflineMode",$global:OfflineMode)
$Runspace.SessionStateProxy.SetVariable("CloudSelection",$global:Cloud)
...
"@
    ...
    $AppendName = ("(" +$CloudSelection + ")")
    $syncHash.Window.Title = ('M365 Endpoint Network Checker ' + $AppendName)
    ...
    $OnPurposeError = try{ Get-M365Hash -Selection test}Catch{$funcerror = $_.exception.message}

    @('WorldWide','DoD','GCC','GCC High','China','Germany') | ForEach-object {$syncHash.startCloudListBox.Items.Add($_)} | Out-Null

    $syncHash.startCloudListBox.SelectedItem = $CloudSelection
    ...
    If($DebugMode){
        ...
        $Functions = ( Get-ChildItem -Path $FunctionPath -Filter *.ps1 -Recurse | Select-String -Pattern "function(.*?){" ) | %{ $_.Line.Substring(9).Replace('{','') } 
        Foreach ($Function in $Functions){
            Update-RunspaceLogging -RunspaceHash $syncHash -Message ("Function loaded in runspace: {0}" -f $Function )
        }
    }
    #========================
    # EVENT HANDLERS
    ...
    $syncHash.Run.Add_Click({
            #RunspacePing -syncHash $syncHash -count $syncHash.Count1.SelectedItem.Content -TenantName $syncHash.tenantName.Text -TargetBox "Logging"
